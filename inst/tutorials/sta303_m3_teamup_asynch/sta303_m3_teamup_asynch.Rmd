...
packages_needed <- c("tidyverse", "learnr", "lme4")
...
```{r, echo=FALSE, fig.align='center', out.width="20%", fig.show='hold', fig.align = 'center'}
knitr::include_graphics(c("images/rose-varieties.png", "images/greenhouse.png"))
```

This Team Up! activity uses the data and ideas from the rose  example we started to explore in synchronous class on Wednesday (Feb 9). I have improved the description a little so it hopefully makes sense alone, without my additional commentary.
...
1. Put `variety` on your x-axis, `profit` on your y, and colour by `greenhouse`.
...
question("What is the correct answer?",
         ...
         answer("D"),
         message = "We want to interpret variety, so it should be a fixed effect. We expect roses from the same greenhouse to potentially be more similar to each other than roses across greenhouses due to the different levels of modernization across them, i.e., correlated. We don't want to interpret greenhouse effects, and it won't help us make predictions for new greenhouses, but we shouldn't ignore the fact that it violates our assumptions of linear regression. Thus, we'll make greenhouse a random effect. As each greenhouse also has roses of each variety, we're interested in accounting for interactions between greenhouse and variety as well."
)
```
...
question("What is the correct answer?",
         ...
         answer("C", correct = TRUE),
         answer("D"),
         message = "8, because there are 8 plots of each vareity in each greenhouse."
)
```
...
question("What is the correct answer?",
         ...
         answer("C"),
         answer("D"),
         message = "The interaction of a fixed effect and a random effect is ALSO a random effect, so A is not tue as only $\alpha_i$ is a fixed effect."
)
```
...
_(See the little code box below to get the hints for the main task. I was having some issues getting the hints to show up because they are kind of long.)_

<br>

```{r modelslong, exercise = TRUE, exercise.lines = 20}
...
```{r modelslongh, exercise = TRUE}
# Press hint here for the relevant hints for above. The rendering is just not working out for me.
```


```{r modelslongh-hint-1}
# Data
...
question("What is the correct answer?",
         ...
         message = "Variety should be one column, but here the levels of vareity are split across several columns."
)
```
