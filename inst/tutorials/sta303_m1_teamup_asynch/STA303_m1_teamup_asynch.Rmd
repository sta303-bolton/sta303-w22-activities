---
title: "STA303: M1 tutorial activity"
subtitle: "Recap and ANOVA"
...
This is a revision task based on the Module 1 Team Up! activity. There are no bonus points for completing the task asynchronously, but I hope it might be a useful revision tool for you.
...
B. Linear regression does not seem appropriate due to the equality of variance assumption being violated.  
C. Linear regression does not seem appropriate due to the linearity assumption being violated.  
D. Linear regression does not seem appropriate due to the independence assumption being violated.  
**
...
Restrict the data to just the seasons from 2010. (If you don't know how to do this, there is a hint). Run a one sample t-test (with `var.equal = TRUE` to test the claim from the TV critic. 
...
```{r onetest-hint-2}
# What is your null hypothesis here? Remember that you use mu = hypothesised value (insert value) to represent your hypothesised value
```

```{r include = FALSE, eval = FALSE}
just2010 <- filter(tv_data_edit, decade == "2010")
t.test(just2010$av_rating, mu = 8.1, var.equal = TRUE)
```
...
D. The `t.test` function produces an error due to a violation of Normality assumption.
**

If you get an error saying your data is missing, add the following code to the beginning of the code chunk to call it again.
...
question("What is the correct answer?",
         ...
         answer("C"),
         answer("D"),
         message = "Our p-value is large (0.64) so we have no evidence against our null hypothesis, which is the claim that the average (average) season rating is 8.1."
)
```
...
tv_data_edit %>% 
  group_by(decade) %>% 
  summarise(a = mean(av_rating), b = var(av_rating), c = sd(av_rating), 
            d = var(av_rating)^2, e = max(av_rating), .groups = "keep")
```
...
B. We have very strong evidence (p-value = 5.79e-08) against the hypothesis that the mean season average ratings are the same in each decade.   C. We have very very strong evidence (p-value = 5.79e-08) that at least one of the mean season average ratings is different from the others.  
...
question("What is the correct answer?",
         ...
         answer("C"),
         answer("D"),
         message = "The correct p-value to interpret is 5.79e-08. You'll see it on the bottom line of the lm output (the one that starts with F-statistic, which is our test statistics for ANOVA. It is a very small number (0.0000000579), and the smaller the p-value the stronger the evidence AGAINST our null hypothesis. Being very careful with our language here, we never want to say evidence FOR the alternative."
)
```
...
CSC108 has 5 sections this semester (L0101, L0201, L0301, L5101, L5102). Suppose the course coordinator hires you to help them do some statistical analysis on their course. They are interested in knowing whether or not all the sections have the same mean score on the first test. The dataset they give you has three columns: student_id, section, score.

Which of the following analyses could you conduct to answer this question? Tick all that apply.

  
Test 1: One-way anova

Test 2: Linear regression with `score` as the response and 4 dummy variables indicating sections: L0201, L0301, L5101, L5102.

Test 3: Linear regression with formula `score` ~ `section`
  
Test 4: Independent t-test
...
question("What is the correct answer?",
         ...
         answer("C"),
         answer("D"),
         message = "As there are 5 sections and we're interested in the question 'do all the sections have the same mean?' we need a test that compares more than two groups. This rules out tests 4 and 5 (and so options C and D). 1 through 3 are all appropriate as this is what they are for. Specifically, One-way anova is a special type of linear regression, where we just have a single categorical predicator of more than two levels. Test 2 and 3 will be equivalent as R will convert Test 3 into Test 2 when it makes the model matrix. We could just make it by hand and feed those indicator variables in as the predictors."
)
```
