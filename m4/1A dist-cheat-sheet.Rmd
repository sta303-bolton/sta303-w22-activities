...
## Poisson
Link: [3.3.6 Poisson Random Variable](https://bookdown.org/roback/bookdown-BeyondMLR/ch-distthry.html#poisson-random-variable)
...
`dpois()` is the function to look at. Run ?dpois in your console for the help information. See the reading for more.
...
`rpois()` is the function to look at. Run ?rpois in your console for the help information.

```{r}
# simulate the data and store it in a convenient way to plot
rand_pois <- tibble(y = rpois(n = 1000, lambda = 5))

# plot
rand_pois %>% 
  ggplot(aes(x = factor(y))) +
  geom_bar() + # appropriate for discrte count of successes
  labs(title = "Number of events per interval",
       subtitle = "1000 simulations",
         x = "Number of events",
       y = "Count") +
  theme_minimal()
...
# [Optional revision] Distributions used in testing
